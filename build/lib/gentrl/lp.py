...
class LP(nn.Module):
    ...
    def sample(self, num_samples, sample_descr, conds=None):
        ...
        if self.tt_type == 'usual':
            ...
            cur_suf = tf.tile(cur_suf[None, :, :], [num_samples, 1, 1])
        ...
        if self.tt_type == 'usual':
            ...
            pref_tf = tf.tile(pref_tf[None, :, :], [num_samples, 1, 1])
        ...
        for i, (action, var, core, suf, prev_contr_vect) in enumerate(
            ...
            if action == 's':
                ...
                weights = tf.reduce_sum(tf.reduce_sum(weights, axis=-1)) + sefl.eps
                ...
                if var[0] == 'd':
                    ...
                    cur_samples = cur_means + tt.math.exp(
    ...
    def reinit_from_data(self, data, var_types=None):
        ...
        for i, var_descr in enumerate(self.distr_descr):
            ...
            if tf.reduce_sum(tf.math.isnan(data[:, i])) == data.shape[0]:
            ...
            if var_descr[0] == 'd':
                ...
                cur_log_stds = tf.math.log(cur_log_stds.float() + self.eps) / 2
