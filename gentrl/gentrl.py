...
#zz import torch.optim as optim
...
class GENTRL(tf.keras.Model):
    ...
    def get_elbo(self, x, y):
        ...
        means, log_stds = tf.split(self.enc.encode(x), num_or_size_splits=2, axis=1)
        #zz latvar_samples = (means + torch.randn_like(log_stds) *
        #zz                   torch.exp(0.5 * log_stds))
        latvar_samples = (means + tf.random.normal(shape=log_stds.shape, dtype=log_stds.dtype) *
        ...
        normal_distr_hentropies = tf.reduce_mean(log(2 * pi) + 1 + log_stds, axis=1)
                ...
                if to_reinit:
                    if (buf is None) or (buf.shape[0] < 5000):
                        ...
                        means, log_stds = tf.split(enc_out,
                                                      ...
                                                      axis=1)
                        z_batch = (means + tf.random.normal(shape=log_stds.shape, mean=0.0, stddev=1.0) *
                                   tf.math.exp(0.5 * log_stds))
